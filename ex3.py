...
with col_review:
    ...
    if analyze_clicked:
        if code_input.strip():
            ...
            with st.spinner("🔍 코드를 전체적으로 분석하며 리뷰를 작성 중입니다... (1/2)"):
                ...
                ### 🚨 오류 분석 (오류가 있을 경우)
                (코드에 문법적, 논리적 오류가 있다면 반드시 <span style="color:red; font-weight:bold;">설명 내용</span> 과 같이 HTML 태그를 사용하여 해당 부분을 빨간색으로 강렬하게 강조해서 설명해. 오류가 없다면 이 항목은 생략해.)
    ...
    if st.session_state.code_review:
        with st.container(height=350, border=True):
            st.markdown(st.session_state.code_review, unsafe_allow_html=True)
    else:
        with st.container(height=350, border=True):
...
if st.session_state.debug_data:
    ...
    with debug_col1:
        ...
        # 단계 이동 및 리셋 버튼
        is_last_step = st.session_state.current_step >= len(data) - 1
        button_text = "🔄 처음부터 다시 디버깅" if is_last_step else "⏭️ 다음 라인으로 (단계별 진행)"
